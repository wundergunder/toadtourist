...
-- First, create a sample tour guide if none exists
DO $$
DECLARE
  guide_id uuid;
BEGIN
  -- Check if any tour guide exists
  SELECT id INTO guide_id FROM profiles WHERE role = 'tour_guide' LIMIT 1;
  
  -- If no tour guide exists, create one
  IF guide_id IS NULL THEN
    ...
    -- Get the guide_id from the newly created profile
    SELECT id INTO guide_id FROM profiles WHERE email = 'guide@example.com' LIMIT 1;
    
    -- If still null, create the profile manually
    IF guide_id IS NULL THEN
      SELECT id INTO guide_id FROM auth.users WHERE email = 'guide@example.com' LIMIT 1;
  ...
  -- Now insert experiences using the guide_id
  IF guide_id IS NOT NULL THEN
    -- Insert jungle kayaking experience
...
This tour is suitable for beginners and includes all necessary equipment including life jackets, paddles, and dry bags for your belongings. We recommend bringing sunscreen, insect repellent, a hat, and water.',
    ...
    -- Insert Mayan cooking class experience
    INSERT INTO experiences (id, title, description, price, duration, max_spots, available_spots, image_urls, territory_id, guide_id)
    SELECT 
      'mayan-cooking',
      'Mayan Cooking Class',
...
This experience is suitable for all skill levels and includes all ingredients and equipment. Just bring your appetite and curiosity!',
    ...
    -- Insert waterfall trek experience
    INSERT INTO experiences (id, title, description, price, duration, max_spots, available_spots, image_urls, territory_id, guide_id)
    SELECT 
      'waterfall-trek',
