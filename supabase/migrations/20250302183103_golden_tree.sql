...
-- Enable Row Level Security if tables exist and don't have it enabled
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'referral_links') THEN
  ...
  IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'commissions') THEN
...
DO $$
BEGIN
  -- Drop policies for referral_links if they exist
  IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'referral_links' AND policyname = 'Users can view their own referral links') THEN
    DROP POLICY "Users can view their own referral links" ON referral_links;
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'referral_links' AND policyname = 'Hotel operators can create their own referral links') THEN
    DROP POLICY "Hotel operators can create their own referral links" ON referral_links;
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'referral_links' AND policyname = 'Hotel operators can update their own referral links') THEN
    DROP POLICY "Hotel operators can update their own referral links" ON referral_links;
  END IF;
  
  -- Drop policies for referrals if they exist
  IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'referrals' AND policyname = 'Hotel operators can view their own referrals') THEN
    DROP POLICY "Hotel operators can view their own referrals" ON referrals;
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'referrals' AND policyname = 'Admins can view all referrals') THEN
    DROP POLICY "Admins can view all referrals" ON referrals;
  END IF;
  
  -- Drop policies for commissions if they exist
  IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'commissions' AND policyname = 'Hotel operators can view their own commissions') THEN
    DROP POLICY "Hotel operators can view their own commissions" ON commissions;
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'commissions' AND policyname = 'Admins can view all commissions') THEN
    DROP POLICY "Admins can view all commissions" ON commissions;
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'commissions' AND policyname = 'Admins can update commissions') THEN
    DROP POLICY "Admins can update commissions" ON commissions;
  END IF;
END $$;
...
CREATE POLICY "Hotel operators can delete their own referral links"
  ON referral_links
  FOR DELETE
  TO authenticated
  USING (
...
DROP TRIGGER IF EXISTS create_hotel_operator_referral_link ON profiles;
